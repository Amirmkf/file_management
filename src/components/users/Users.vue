...
<template>
  <div class="users">
    <table>
      ...
      <tr v-for="user in $store.state.users.users" :key="user.username">
        <td>{{ user.username }}</td>
        <td>{{ user.creationTime }}</td>
        <td>{{ user.lastModifier }}</td>
        <td>{{ user.lastModificationTime }}</td>
        <td>
          <button class="filled delete" @click="removeUser(user.username)">
            <svg
          ...
          <router-link
            :to="{ name: 'editUser', params: { username: user.username } }"
            class="button"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
