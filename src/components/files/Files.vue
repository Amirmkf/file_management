<script>
const getAllFiles = (storage) => {
  let allFiles = [];

  const extractFiles = (folder) => {
    folder.files.forEach((file) => allFiles.push(file));

    folder.folders.forEach((subFolder) => {
      extractFiles(subFolder);
    });
  };

  storage.files.forEach((file) => {
    allFiles.push(file);
  });

  storage.folders.forEach((rootFolder) => {
    extractFiles(rootFolder);
  });

  return allFiles;
};

export default {
  computed: {
    activeUser() {
      return this.$store.state.users.activeUser;
    },
    files() {
      return getAllFiles(this.activeUser.storage);
    },
  },
};
</script>

<template>
  <div class="files">
    <table>
      <thead>
        <tr>
          <th>{{ $t("table.name") }}</th>
          <th>{{ $t("table.owner") }}</th>
          <th>{{ $t("table.size") }}</th>
          <th>{{ $t("table.uploadTime") }}</th>
        </tr>
      </thead>
      <tr v-for="file in files">
        <td>
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M20.5 10.19H17.61C15.24 10.19 13.31 8.26 13.31 5.89V3C13.31 2.45 12.86 2 12.31 2H8.07C4.99 2 2.5 4 2.5 7.57V16.43C2.5 20 4.99 22 8.07 22H15.93C19.01 22 21.5 20 21.5 16.43V11.19C21.5 10.64 21.05 10.19 20.5 10.19Z"
              fill="rgba(128, 236, 243, 1)"
            />
            <path
              d="M15.7997 2.21048C15.3897 1.80048 14.6797 2.08048 14.6797 2.65048V6.14048C14.6797 7.60048 15.9197 8.81048 17.4297 8.81048C18.3797 8.82048 19.6997 8.82048 20.8297 8.82048C21.3997 8.82048 21.6997 8.15048 21.2997 7.75048C19.8597 6.30048 17.2797 3.69048 15.7997 2.21048Z"
              fill="rgba(128, 236, 243, 1)"
            />
          </svg>
          {{ file.name }}
        </td>
        <td>{{ file.owner }}</td>
        <td>{{ file.size }}</td>
        <td>{{ file.uploadTime }}</td>
      </tr>
    </table>
  </div>
</template>
